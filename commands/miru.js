...
const MIRU_PREFIX = 'miru';
const MIRU_IMAGE = 'https://s1.pearlcdn.com/KR/Upload/News/e49018f5fd520260109145903041.png';

module.exports = {
    data: {
        name: 'miru',
        description: 'Crear party Mirumok Dekia',
    },

    async execute(interaction, gruposMiru, saveMiru, client) {
        const now = new Date();
        ...
        const modal = new ModalBuilder()
            .setCustomId(`${MIRU_PREFIX}_create_modal`)
            .setTitle('Crear Party (Mirumok Dekia)')
            .addComponents(
        ...
        await interaction.showModal(modal); // [web:1]
    },

    async handleInteraction(interaction, gruposMiru, saveMiru, client) {
        ...
        function resolveGrupoFromInteraction(interaction) {
            const idFromCustom = getPartyIdFromCustomId(interaction.customId, [`${MIRU_PREFIX}_add_select`, `${MIRU_PREFIX}_kick_select`]);
            if (idFromCustom && gruposMiru.has(idFromCustom)) return gruposMiru.get(idFromCustom);

            const mid = interaction.message?.id;
            if (mid && gruposMiru.has(mid)) return gruposMiru.get(mid);

            if (interaction.isModalSubmit?.()) {
                const parts = (interaction.customId || '').split('_');
                const pid = parts[2];
                if (pid && gruposMiru.has(pid)) return gruposMiru.get(pid);
            }
            return null;
        }

        // ------- CREAR PARTY DESDE MODAL -------
        if (interaction.isModalSubmit() && interaction.customId === `${MIRU_PREFIX}_create_modal`) {
            const motivo = 'Mirumok Dekia';
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '', value: `Ataque total : \`\`\`1565+  (límite 1595+)\`\`\``, inline: true },
                    { name: '', value: `Defensa      : \`\`\`615+\`\`\``, inline: true },
            ...
            const joinBtn = new ButtonBuilder().setCustomId(`${MIRU_PREFIX}_join`).setLabel('Unirme').setEmoji('✋').setStyle(ButtonStyle.Success);
            const leaveBtn = new ButtonBuilder().setCustomId(`${MIRU_PREFIX}_leave`).setLabel('Salir').setEmoji('🚫').setStyle(ButtonStyle.Danger);
            const editBtn = new ButtonBuilder().setCustomId(`${MIRU_PREFIX}_edit`).setLabel('Editar').setStyle(ButtonStyle.Secondary);
            const addBtn = new ButtonBuilder().setCustomId(`${MIRU_PREFIX}_add`).setLabel('Agregar a la Party').setStyle(ButtonStyle.Secondary);
            const kickBtn = new ButtonBuilder().setCustomId(`${MIRU_PREFIX}_kick`).setLabel('Eliminar de la Party').setStyle(ButtonStyle.Secondary);
        ...
        // Resolución grupo para el resto de interacciones
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId === `${MIRU_PREFIX}_join`) {
            ...
            if (interaction.customId === `${MIRU_PREFIX}_leave`) {
                if (!grupo.miembros.some(u => u.id === user.id)) {
            ...
            if (interaction.customId === `${MIRU_PREFIX}_edit`) {
                ...
                const modal = new ModalBuilder()
                    .setCustomId(`${MIRU_PREFIX}_modal_${grupo.mensajeId}`)
                    .setTitle('Editar party (Mirumok Dekia)')
                    .addComponents(
                        new ActionRowBuilder().addComponents(
                            new TextInputBuilder()
                                .setCustomId('hora')
            ...
            if (interaction.customId === `${MIRU_PREFIX}_add`) {
                if (!isOwner) return interaction.reply({ content: 'Solo el creador puede hacer esto.', flags: MessageFlags.Ephemeral }).catch(() => { });
                ...
                const userSelect = new UserSelectMenuBuilder()
                    .setCustomId(`${MIRU_PREFIX}_add_select:${grupo.mensajeId}`)
            ...
            if (interaction.customId === `${MIRU_PREFIX}_kick`) {
                if (!isOwner) return interaction.reply({ content: 'Solo el creador puede hacer esto.', flags: MessageFlags.Ephemeral }).catch(() => { });
                ...
                const userSelect = new UserSelectMenuBuilder()
                    .setCustomId(`${MIRU_PREFIX}_kick_select:${grupo.mensajeId}`)
        ...
        if (interaction.isUserSelectMenu() && interaction.customId.startsWith(`${MIRU_PREFIX}_add_select:`)) {
            const partyMsgId = interaction.customId.split(':')[1];
            grupo = gruposMiru.get(partyMsgId);
        ...
        if (interaction.isUserSelectMenu() && interaction.customId.startsWith(`${MIRU_PREFIX}_kick_select:`)) {
            const partyMsgId = interaction.customId.split(':')[1];
            grupo = gruposMiru.get(partyMsgId);
            if (!grupo) return interaction.reply({ content: 'Este grupo ya no está activo.', flags: MessageFlags.Ephemeral }).catch(() => { });
            if (interaction.user.id !== grupo.creador.id) {
        ...
        if (interaction.isModalSubmit() && interaction.customId === `${MIRU_PREFIX}_modal_${grupo.mensajeId}`) {
            if (interaction.user.id !== grupo.creador.id) {
                return interaction.reply({ content: 'Solo el creador puede hacer esto.', flags: MessageFlags.Ephemeral }).catch(() => { });
            }
